...
- **简历 PDF 自动识别**：上传简历 PDF（pypdf 提取文本层 + LLM 结构化解析），自动填好目标岗位、技能栈、项目经历（含量化成果与深挖点），不用手动录入
...
- **测试与评估**：23 项 pytest（路由 / 专员权限 / 记忆 / RAG / prep-post 面试闭环 / 档案 / 复盘 / 学习教练 / 简历解析 / 面试官生成）+ 3 个离线 eval 脚本
...
| 自动化测试 | pytest | **23/23 通过** | 路由/权限/记忆/RAG/闭环/档案/复盘/教练/简历解析 |
...
7. **工程细节与反思**：embedding 模型单例缓存（修掉每次检索重复加载模型的问题）、LLM 重试与多级降级、23 项测试覆盖核心链路；诚实说明边界（并发与安全是二期方向）。
...
针对本项目的技术栈（多 Agent 编排、RAG、Function Calling、Python、前端）与「第一次找 AI 应用开发实习」的阶段，按优先级推荐（书名均可在微信读书 App / 网页版搜索到）：

| 优先级 | 书名 | 为什么读 |
| --- | --- | --- |
| ★★★ | 《大模型应用开发》（鲍亮 等） | 把 RAG / Agent / 提示词工程 / 微调 / 应用架构串成完整体系，面试讲本项目时更有条理 |
| ★★★ | 《AI Agent智能体开发实践》（邓立国 等） | 讲透 LangChain 智能体、MCP、工具编排、多智能体协作；与手写 Function Calling 循环对照，能讲清「为什么手写而不是套框架」的取舍 |
| ★★★ | 《流畅的Python（第2版）》 | 生成器、装饰器、内存模型、异步——本项目 Agent 循环、LLM 重试容错、FastAPI 底层都建立在这些概念上，也是面试 Python 八股的高频区 |
| ★★ | 《MySQL必知必会（第2版）》 | 数据库高频考点，书很薄，两周可过 |
| ★★（进阶） | 《数据密集型应用系统设计》 | 索引、存储、一致性、分布式系统设计——能把 RAG 检索、SQLite 存储方案讲深，面试系统设计加分项 |
| ★（补充） | 《动手学深度学习（第二版）》 | 李沐著，官网免费阅读（zh.d2l.ai），把 PyTorch 从「了解」补到能说清训练与评估 |
