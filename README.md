...
| 优先级 | 书名 | 为什么读 |
| --- | --- | --- |
| ★★★ | 《大模型应用开发》 | RAG / Agent / 提示词工程 / 微调 / 应用架构全链路 |
| ★★★ | 《AI Agent智能体开发实践》 | LangChain / MCP / 工具编排，对照手写 Function Calling 循环 |
| ★★★ | 《流畅的Python（第2版）》 | 生成器 / 装饰器 / 内存模型 / 异步，Python 八股 + Agent 循环底层 |
| ★★ | 《MySQL必知必会（第2版）》 | 数据库高频考点 |
| ★★（进阶） | 《数据密集型应用系统设计》 | 索引 / 存储 / 一致性，把 RAG 检索与存储方案讲深 |
| ★（补充） | 《动手学深度学习（第二版）》 | 官网免费（zh.d2l.ai），补 PyTorch |
